...
> **声明**：本人为编程小白，本项目由本人向 AI 提交需求，经多个 AI接力协作完成（因免费积分有限）。<br>
>描述可能存在疏漏或与实际情况不符，敬请谅解。
> 
> **注意**：关闭了Issue提交和PR，因本人暂无能力处理，感谢理解。
> 
> 开发初始时用了《超级机器人大战OG2》的音乐调试，然后不知怎么地命中了可用地址，一度以为开发思路没问题，导致后续扩大到支持其他游戏音乐的大量修复尝试皆以失败告终。<br>
> 后面将逻辑重新构建，目前插件仅能播放部分游戏的 `.minigsf `音乐（有时会有噪音），详见以下个人测试列表。
> 
> **欢迎有能力/感兴趣的开发者 Fork 并修复！感谢！**
...
- 插件未对 `gsflib `外部库引用做充分处理，可能影响兼容性。
- 当前仅能播放部分游戏的音乐，个人测试结果如下：（**非完整测试列表**，仅选了我自己喜欢的几首音乐，所以可能会出现同一个游戏中部分音乐可播放部分不行的问题）
    1. `公主联盟` 正常播放
    2. `塞尔达缩小帽` 正常播放
    3. `宝可梦蓝宝石` 正常播放
    4. `正邪幻想曲Zero` 正常播放
    5. `约束之地` 正常播放
    6. `超级机器人大战OG2` 大部分时候都是噪音 （重构之后就不支持了，太搞了）
    7. `逆转裁判` 正常播放
    8. `铸剑物语` 没有声音
    9. `铸剑物语3` 没有声音
    10. `露娜传奇` 没有声音
...
2. 选择配置：`Release`，平台：`x86`（**必须**，AIMP 为 32 位程序）
3. 生成解决方案（`Ctrl+Shift+B` 或菜单“生成”）
4. 将编译生成的 `aimp_gsf.dll` 复制到 AIMP 插件目录（例如 `C:\Program Files （x86）\AIMP\Plugins\aimp_gsf`）
5. 重启 AIMP 播放器，程序检测不到插件的话需要手动到插件界面勾选开启

### 运行时依赖

- 系统需具备 `zlib.dll`（插件动态加载），若缺失请从 [zlib.net]（https://zlib.net/） 获取并置于系统 PATH 或 AIMP 目录下。
...
├── GBA精选.zip     # 我喜欢的GBA音乐
├── gsf_plugin_src-v21-仅支持OG2.zip     # 重构前的版本，仅支持OG2，含预编译 DLL
├── README.md     # 本文档
...
### v1 （2026-07-12）
- 发觉思路有问题，将模拟器逻辑重构。实测仅支持部分游戏音乐播放，大量修复尝试以失败告终。

### v47 （2026-07-11）
- 尝试扩展支持其他 `.minigsf` 游戏音乐，但未能成功。被偶然命中的可用地址08002DAC带飞了思路，这期间的修复全是无用功。
- 主要修改GbaEmulator.cpp/.h文件，其他基本无变动。
...
### v1 （2026-07-04）
- 初始版本，基于 `foo_input_gsf` 代码移植至 AIMP SDK。
...
- **GSF 格式**：属于 PSF 家族，针对 GBA 音频驱动压缩存储，miniGSF 依赖外部 `gsflib` 库。
...
- [kode54](https://www.foobar2000.org/) – foobar2000 `foo_gsf` 插件作者
